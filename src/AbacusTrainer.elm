...
target_area picture_address ( w, h ) current_state = 
    g [Svg.Events.onClick ( Signal.message picture_address ( ClickedRefresh ) )]
      [
       ...
       ,( rect  [ x "750", y "120",   width "120", height "120", rx "15", ry "15",  fill "lightBlue" ] []  )
       ,( text' [ x "750", y "160", fontSize "55" ] [ text ( toString ( current_state.score ) ) ] )
...
resetBallLocation n ball = { ball| x = ( n * 80 + 400 ) , y = 60 } 
...
view address ( w, h ) current_state =
  ...
  [
   ...
   , div [] [ Html.ol [] [ 
        ( Html.li [] [ Html.text  "Click in the Green square to get a new target number to click."  ] )
      , ( Html.li [] [ Html.text  "Click on the ball with the number shown in the Green square "    ] )
      , ( Html.li [] [ Html.text  "The light Blue square indicates your score."                     ] )
      , ( Html.li [] [ Html.text  "The numbers on the balls become smaller as you get more correct" ] )
      , ( Html.li [] [ Html.text  "And they get bigger when you make mistakes."                     ] )
      ]
    ]
